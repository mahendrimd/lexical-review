...
async function openDeveloperDetails(page: Page) {
...
test("R1 insertion continuation keeps one identity via native typing", async ({
  ...
  await expect(page.getByTestId("selected-details")).toHaveCount(0);

  await page.getByTestId("generate-evidence").click();
  await expect(page.getByText(DEVELOPER_DETAILS_LABEL)).toBeVisible();
  await expect(page.getByTestId("selected-details")).toBeHidden();
...
test("merge paragraph creates a structural proposal that can be rejected", async ({
  ...
  await expect(page.getByRole("status")).toContainText("pending proposal");
  await expect(page.getByTestId("proposal-item")).toContainText("merge");
...
test("switching scenarios resets edits without reporting an outcome", async ({
  ...
  await expect(page.getByTestId("proposal-item")).toHaveCount(0);
  await expect(page.getByText(DEVELOPER_DETAILS_LABEL)).toHaveCount(0);
  await expect(page.getByTestId("outcome-pane")).toHaveCount(0);
  await expect(page.getByTestId("evidence-status")).toHaveText("Not generated");
...
test("capability surface is labelled non-normative with live/docs leads", async ({
  ...
  await expect(page.getByTestId("capability-label")).toHaveCount(0);
  await page.getByTestId("generate-evidence").click();
  await openDeveloperDetails(page);
